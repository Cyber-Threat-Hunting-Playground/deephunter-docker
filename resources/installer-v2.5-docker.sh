...
export GITHUB_REPO="${GITHUB_REPO:-Cyber-Threat-Hunting-Playground/deephunter}"
...
chown -R $(id -nu):$(id -ng) $(dirname ${APP_PATH})
chmod -R 755 $(dirname ${APP_PATH})
mkdir -p ${APP_PATH}
mkdir -p $(dirname $VENV_PATH)
chown -R $(id -nu):$(id -ng) $(dirname $VENV_PATH)
...
apt-get install -y --no-install-recommends git wget gnupg dos2unix \
    python3-venv python3-wheel python3-dev \
    build-essential pkg-config \
    libmariadb-dev \
    apache2 apache2-utils libapache2-mod-wsgi-py3 >> /tmp/install.log 2>&1
...
##mysql >> /tmp/install.log 2>&1 <<-EOSQL
...
cd /tmp/
DEEPHUNTER_TARBALL="v${DEEPHUNTER_VERSION}.tar.gz"
DEEPHUNTER_URL="https://github.com/${GITHUB_REPO}/archive/refs/tags/${DEEPHUNTER_TARBALL}"
if [ -f "/resources/${DEEPHUNTER_TARBALL}" ]; then
    cp "/resources/${DEEPHUNTER_TARBALL}" "./${DEEPHUNTER_TARBALL}" >> /tmp/install.log 2>&1
else
    wget -q --tries=3 --retry-connrefused --waitretry=5 \
        -O "${DEEPHUNTER_TARBALL}" "$DEEPHUNTER_URL" >> /tmp/install.log 2>&1 || {
        echo -e "[\033[31mfailed\033[0m]" | tee -a /tmp/install.log
        echo "ERROR: Failed to download DeepHunter from GitHub." | tee -a /tmp/install.log
        echo "If behind a proxy/firewall, download manually and place in resources/:" | tee -a /tmp/install.log
        echo "  curl -L -o resources/${DEEPHUNTER_TARBALL} $DEEPHUNTER_URL" | tee -a /tmp/install.log
        echo "Then rebuild." | tee -a /tmp/install.log
        exit 1
    }
fi
tar xzf "${DEEPHUNTER_TARBALL}" -C "${APP_PATH}" --strip-components=1 >> /tmp/install.log 2>&1
find "${APP_PATH}" -name '*.sh' -exec dos2unix -q {} + >> /tmp/install.log 2>&1
echo -e "[\033[32mdone\033[0m]" | tee -a /tmp/install.log
...
chown :$SERVER_USER $APP_PATH/deephunter/wsgi.py >> /tmp/install.log 2>&1
chown -R :$SERVER_USER $APP_PATH/plugins/ >> /tmp/install.log 2>&1
chmod g+s $APP_PATH/plugins/ >> /tmp/install.log 2>&1
echo -e "[\033[32mdone\033[0m]" | tee -a /tmp/install.log
...
a2enmod headers >> /tmp/install.log 2>&1
...
a2enmod ssl >> /tmp/install.log 2>&1

# improve your encryption by creating a strong DH Group, and enable Perfect Forward Secrecy:
cp ${APP_PATH}/install/etc/apache2/conf-available/ssl-params.conf /etc/apache2/conf-available/
openssl dhparam -out /etc/ssl/certs/dhparam.pem 2048 >> /tmp/install.log 2>&1
a2enconf ssl-params >> /tmp/install.log 2>&1

# Enable site in HTTPS
sed -i "s|/data/deephunter|${APP_PATH}|" ${APP_PATH}/install/etc/apache2/sites-available/deephunter-ssl.conf
cp ${APP_PATH}/install/etc/apache2/sites-available/deephunter-ssl.conf /etc/apache2/sites-available/
a2ensite deephunter-ssl >> /tmp/install.log 2>&1
...
cp ${APP_PATH}/install/etc/default/celery /etc/default/celery
...
##echo 'd /var/run/celery 0755 celery celery' | tee /etc/tmpfiles.d/celery.conf >> /tmp/install.log 2>&1
...
cp ${APP_PATH}/install/etc/systemd/system/celery.service /etc/systemd/system/celery.service

# Reload services and enable them:
##systemctl daemon-reload >> /tmp/install.log 2>&1
##systemctl enable celery.service >> /tmp/install.log 2>&1
##systemctl start celery.service >> /tmp/install.log 2>&1

echo -e "[\033[32mdone\033[0m]" | tee -a /tmp/install.log

# Restart services to apply all changes
##echo -n -e "[\033[90mINFO\033[0m] RESTARTING SERVICES ............................. " | tee -a /tmp/install.log
##systemctl restart apache2 >> /tmp/install.log 2>&1
##systemctl restart redis >> /tmp/install.log 2>&1
