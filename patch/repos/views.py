...
def import_repo_select_analytics(request, repo_id):
    ...
    if "github.com" in repo.url:
        ...
        connector_method = 'get_github_contents'
    elif "bitbucket.org" in repo.url:
        connector = all_connectors.get('bitbucket')
        connector_method = 'get_bitbucket_contents'

    if connector is None:
        platform = "GitHub" if "github.com" in repo.url else "Bitbucket" if "bitbucket.org" in repo.url else "unknown"
        add_error_notification(
            f"The {platform} connector is not installed. "
            f"Install it from the Connectors Catalog before importing this repo."
        )
        return HttpResponseRedirect(reverse('list_repos'))

    contents = getattr(connector, connector_method)(repo)
    context = {
        "repo": repo,
        "contents": contents,
    }
