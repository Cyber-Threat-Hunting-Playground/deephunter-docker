...
def db_analyticsrunintodaycampaign(request):
    ...
    for i in range(30, 0, -1):
        ...
        sparkline.append((c.nb_queries or 0) if c else 0)
...
def db_campaign_completion(request):
    ...
    if analytics_run == analytics_target:
        code = f"""<h3>Campaign completion<br />(run/target)</h3>
        <div class="num_green"><a href="/qm/managecampaigns">{analytics_run}/{analytics_target}</a></div>"""
    else:
        code = f"""<h3>Campaign completion<br />(run/target)</h3>
        <div class="num_red"><a href="/qm/managecampaigns">{analytics_run}/{analytics_target}</a></div>"""
    return HttpResponse(code)
...
def db_highestweightedscoretoday(request):
    ...
    if not campaign:
        code = """
        <h3>Endpoint with highest weighted relevance today</h3>
        <div class="num">
            <a href="/reports/endpoints/">0</a>
        </div>
    """
        return HttpResponse(code)
    ...
    if highestweightedscore is None:
        code = """
        <h3>Endpoint with highest weighted relevance today</h3>
        <div class="num">
            <a href="/reports/endpoints/">0</a>
    ...
    """
        ...
        <h3>Endpoint with highest weighted relevance today</h3>
        <div class="num">
            <a href="/reports/endpoints/">{highestweightedscore['total_weighted_score']}</a>
        </div>
    """
...
def db_top_endpoint_distinct_analytics(request):
    ...
    endpoint = top_endpoints.first()

    if endpoint is None:
        code = f"""<h3>Most distinct analytics on single endpoint</h3>
            <div class="num"><a href="/reports/endpoints_most_analytics">0</a></div>
            """
    else:
        code = f"""<h3>Most distinct analytics on single endpoint</h3>
            <div class="num"><a href="/reports/endpoints_most_analytics">{endpoint['analytics_count']}</a></div>
            """
    return HttpResponse(code)
